  ...
  try {
    ...
    const empSql = `
      SELECT
        ...
        (SELECT NAME FROM HR_EMP_MASTER hod WHERE hod.EMPCODE = TO_CHAR(h.RPT_OFFICER))  AS "hod_nm"
    ...
    // Aggregate leave balance from ALL_LEAVE_BAL_V (mirrors FastAPI's unfiltered
    // SUM(balance) in repositories/user_repository.py get_dashboard — no YEAR filter).
    const balRow = (
